...
def main(unused_argv):
    ...
    with tf.Session() as sess:
        policy = DQN(sess,0,info_state_size**0.5,num_actions,hidden_layers_sizes,**kwargs)
        rollout = DQN(sess, 0, info_state_size ** 0.5, num_actions, hidden_layers_sizes, **kwargs)
        # agents = [DQN(sess, _idx, info_state_size,
        #                   num_actions, hidden_layers_sizes, **kwargs) for _idx in range(2)]  # for self play

        sess.run(tf.global_variables_initializer())
        #read the earlier model
        #policy.restore()
        #rollout.restore()
        agents = [MCTSAgent(policy, rollout, playout_depth=10,n_playout = 100
                      ), agent.RandomAgent(1)]
